  ...
  const adapter: DatabaseAdapter & { close: () => void } = {
    ...
    deviceHealthSnapshots: {
      ...
      async getByTenantId(tenantId: string, sinceDate: string): Promise<DeviceHealthSnapshot[]> {
        const rows = db
          .prepare('SELECT * FROM device_health_snapshots WHERE tenant_id = ? AND snapshot_date >= ? ORDER BY snapshot_date ASC')
          .all(tenantId, sinceDate) as Record<string, unknown>[];
      ...
      async deleteOlderThan(cutoffDate: string): Promise<number> {
        const result = db.prepare('DELETE FROM device_health_snapshots WHERE snapshot_date < ?').run(cutoffDate);
        return result.changes;
      },
    ...
    fleetAppInventory: {
      async replaceForDate(tenantId, snapshotDate, rows) {
        const deleteStmt = db.prepare('DELETE FROM fleet_app_inventory WHERE tenant_id = ? AND snapshot_date = ?');
      ...
      async getLatestForTenant(tenantId, limit = 20) {
        ...
        const rows = db
          .prepare('SELECT * FROM fleet_app_inventory WHERE tenant_id = ? AND snapshot_date = ? ORDER BY device_count DESC LIMIT ?')
      ...
      async deleteOlderThan(cutoffDate: string): Promise<number> {
        const result = db.prepare('DELETE FROM fleet_app_inventory WHERE snapshot_date < ?').run(cutoffDate);
        return result.changes;
      },
    },
    ...
    updateCheckResults: {
      ...
      async getByUserId(
        ...
        return rows.map((row) => ({ ...row, is_critical: !!row.is_critical, is_managed: !!row.is_managed }) as UpdateCheckResultRecord);
      },
      ...
      async setDismissed(ids: string[], userId: string, dismissed: boolean): Promise<number> {
        ...
        const now = new Date().toISOString();
        const result = db
          .prepare(`UPDATE update_check_results SET dismissed_at = ?, updated_at = ? WHERE user_id = ? AND id IN (${placeholders})`)
      ...
      async deleteByIds(ids: string[]): Promise<number> {
        if (ids.length === 0) return 0;
        const placeholders = ids.map(() => '?').join(', ');
        const result = db.prepare(`DELETE FROM update_check_results WHERE id IN (${placeholders})`).run(...ids);
        return result.changes;
      },
      ...
      async deleteOlderThan(cutoffDate: string): Promise<number> {
        const result = db.prepare('DELETE FROM update_check_results WHERE detected_at < ?').run(cutoffDate);
        return result.changes;
      },
    },
    ...
    deploymentDrift: {
      ...
      async getByUserId(
        ...
        const rows = db
          ...
          .all(...params) as Record<string, unknown>[];
        return rows.map((row) => ({ ...row, partial: !!row.partial }) as DeploymentDriftRecord);
      },
      async deleteOlderThan(cutoffDate: string): Promise<number> {
        const result = db.prepare('DELETE FROM deployment_drift_results WHERE scanned_at < ?').run(cutoffDate);
        return result.changes;
      },
    },
    ...
    deviceBiosInfo: {
      ...
      async pruneRemoved(tenantId: string, currentDeviceIds: string[]): Promise<number> {
        if (currentDeviceIds.length === 0) {
          const result = db.prepare('DELETE FROM device_bios_info WHERE tenant_id = ?').run(tenantId);
          return result.changes;
        }
