    ...
    try {
      ...
      try {
        ...
        if (error instanceof DuplicateAppError) {
          // Same app already deployed to this tenant via HKS App Deployment (by any
...
.DESCRIPTION
    Deploys ${appName} ${appVersion} using PSAppDeployToolkit v4
    Generated by HKS App Deployment self-hosted packager
#>
...
${this.getPostCommandsBlock(job, 'postInstallCommands', 'Install-ADTDeployment')}
    ## Create HKS App Deployment detection marker for Intune detection rules
...
${this.getPostCommandsBlock(job, 'postUninstallCommands', 'Uninstall-ADTDeployment')}
    ## Remove HKS App Deployment detection marker
...
`;
  ...
  private getInstallCommand(job: PackagingJob, fileName: string, silentSwitches: string): string {
    ...
    if (ext === '.msix' || ext === '.msixbundle' || installerType === 'msix' || installerType === 'appx') {
  ...
  private getZipInstallCommand(job: PackagingJob, fileName: string, silentSwitches: string): string {
    ...
    return `$zipExtractDir = [System.IO.Path]::Combine($env:TEMP, "HKSDeploy_Zip_" + [System.Guid]::NewGuid().ToString("N").Substring(0, 8))
  ...
  private getUninstallCommand(job: PackagingJob): string {
    ...
    // MSIX/APPX uninstall: remove the provisioned package and any already-registered instances
    const msixUninstallMatch = job.uninstall_command.match(/^MSIX_UNINSTALL:(.+)$/);
    if (msixUninstallMatch) {
      const packageNameEscaped = msixUninstallMatch[1].replace(/'/g, "''");
      return `$packageName = '${packageNameEscaped}'
  ...
  /**
   * Sanitize a version string before interpolating it into single-quoted
   * PowerShell string literals (and PS `<# ... #>` comment blocks).
   *
   * job.version originates from community-submitted winget manifests and
   * must not be trusted as-is: an unescaped single quote breaks out of a
   * PS string literal, and `#>` breaks out of a doc-comment block, either
   * of which lets a malicious manifest inject arbitrary PowerShell that
   * runs as SYSTEM on every device the package is deployed to.
   *
   * Winget versions are effectively free-form, so rather than rejecting
  ...
  private sanitizeVersion(version: string): string {
    ...
    if (!cleaned) {
      throw new Error(`Invalid or unsafe version string: ${JSON.stringify(version)}`);
    }
    return cleaned;
  }

  /**
   * Normalize an architecture string before interpolating it into a
   * PowerShell string literal. Only a known allow-list is accepted -
   * anything else (including injection attempts via a manifest field)
   * falls back to 'x64', matching IntuneUploader.mapArchitecture's
   * fallback for unrecognized values.
   */
  private sanitizeArchitecture(architecture: string | null | undefined): string {
    const known = ['x64', 'x86', 'arm64', 'arm', 'neutral', 'all'];
    const normalized = (architecture ?? '').toLowerCase().trim();
    return known.includes(normalized) ? normalized : 'x64';
  }
  ...
  private normalizeMarkerPath(input: unknown): string {
    const DEFAULT_MARKER_PATH = 'SOFTWARE\\HKS\\Apps';
   ...
   * package_config.psadtConfig.registryMarkerPath, or the HKS App Deployment default
   */
  ...
  /**
   * Generate PowerShell code to create HKS App Deployment registry marker
  ...
  private getRegistryMarkerCreation(job: PackagingJob): string {
    ...
    return `try {
        ...
        Set-ADTRegistryKey -LiteralPath $regPath -Name 'Version' -Value '${appVersion}' -Type String
        ...
        Write-ADTLogEntry -Message "HKS App Deployment detection marker written to $regPath" -Severity 'Success' -Source 'Install-ADTDeployment'
    } catch {
  ...
  /**
   * Generate PowerShell code to remove HKS App Deployment registry marker
  ...
  private getRegistryMarkerRemoval(job: PackagingJob): string {
    ...
    return `try {
        ...
        if (Test-Path -LiteralPath 'Registry::${hiveLongName}\\${markerPath}\\${sanitizedId}' -PathType Container) {
            ...
            Write-ADTLogEntry -Message "HKS App Deployment detection marker removed from $regPath" -Severity 'Success' -Source 'Uninstall-ADTDeployment'
        }
    } catch {
