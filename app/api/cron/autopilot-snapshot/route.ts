/**
 * Autopilot Device Snapshot Cron Route
 *
 ...
 * double-counting - it just re-syncs whatever's still current for the tenant.
 *
...
import { captureDueAutopilotSnapshots } from '@/lib/intune-reports/autopilot';
...
export async function GET(request: Request) {
  ...
  try {
    await captureDueAutopilotSnapshots(BUDGET_MS);
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('Error running Autopilot snapshot cron:', error);
